...
/**
 * 该类通过定时拉取的方式从应用中拉取Metric，不适合大规模集群的布署，现将其${code @Deprecated}掉，<br>
 * 使用${code ＠MetricControllerV2}的方式接收应用上报上来的Metric，然后通过${code
 * MetricFetcherV2}处理。 Fetch metric of machines.
 *
 * @author leyou
 */
@Deprecated
...
public class MetricFetcher {
	...
	public MetricFetcher() {
		...
		fetchService = new ThreadPoolExecutor(cores, cores, keepAliveTime, TimeUnit.MILLISECONDS,
				new ArrayBlockingQueue<>(queueSize), new NamedThreadFactory("sentinel-dashboard-metrics-fetchService",true),
				handler);
