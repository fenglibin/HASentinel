...
public class AuthController {
	...
	public Result<AuthService.AuthUser> login(HttpServletRequest request, HttpServletResponse response, String username,
		...
		/*
		 * If auth.username or auth.password is blank(set in application.properties or
		 * VM arguments), auth will pass, as the front side validate the input which
		 * can't be blank, so user can input any username or password(both are not
		 * blank) to login in that case.
		 */
		...
		// 增加对COOKIE的支持
		Cookie userCookie = new Cookie(SimpleWebAuthServiceImpl.WEB_SESSION_KEY,
				AES.encrypt(username + ":" + username));
