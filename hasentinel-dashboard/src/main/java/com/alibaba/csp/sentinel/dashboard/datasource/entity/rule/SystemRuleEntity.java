...
public class SystemRuleEntity implements RuleEntity {
	...
	public SystemRuleEntityV2 getSystemRuleEntityV2() {
		SystemRuleEntityV2 rule = new SystemRuleEntityV2();
		rule.setId(String.valueOf(this.id));
		rule.setApp(this.app);
		rule.setIp(this.ip);
		rule.setPort(this.port);
		rule.setHighestCpuUsage(this.getHighestCpuUsage());
		rule.setAvgRt(this.avgRt);
		rule.setHighestSystemLoad(this.highestSystemLoad);
		rule.setMaxThread(this.maxThread);
		rule.setQps(this.qps);
		rule.setGmtCreate(this.getGmtCreate());
		rule.setGmtModified(this.getGmtCreate());
		return rule;
	}
}
