...
@Slf4j
@Component
public class ZookeeperSystemRuleStore<T extends SystemRuleEntity> implements RuleRepository<T, Long> {

	@Autowired
	private CuratorFramework zkClient;
	@Autowired
	private Converter<SystemRuleEntity, String> encodeConverter;
	@Autowired
	private Converter<String, SystemRuleEntity> decodeConverter;
	@Autowired
	private Converter<Map<Long, SystemRuleEntity>, String> mapEncodeConverter;
	@Autowired
	private Converter<String, Map<Long, SystemRuleEntity>> mapDecodeConverter;
	...
	public T save(T entity) {
		...
		if (processedEntity != null) {
			String zkPathId = ZookeeperConfigUtils.getSystemRuleZkPath(String.valueOf(entity.getId()));
			String zkPathApp = ZookeeperConfigUtils.getSystemRuleZkPath(entity.getApp());
			byte[] data = null;
			...
			try {
				...
				Map<Long, SystemRuleEntity> entities = (Map<Long, SystemRuleEntity>) findBy(entity.getApp());
				if (entities == null) {
				...
				log.error("将app:" + entity.getApp() + " ID:" + entity.getId() + "的系统规则保存到ZK中发生异常：" + e.getMessage(), e);
			}
		}
	...
	public T delete(Long id) {
		String zkPathId = ZookeeperConfigUtils.getSystemRuleZkPath(String.valueOf(id));
		try {
			...
			Map<Long, SystemRuleEntity> appEntities = (Map<Long, SystemRuleEntity>) findBy(entity.getApp());
			if (appEntities != null) {
			...
			log.error("根据ID：" + id + " 删除系统规则发生异常：" + e.getMessage(), e);
		}
		return null;
	}
	...
	public T findById(Long id) {
		...
		try {
			...
			log.error("根据ID：" + id + " 查询系统规则发生异常：" + e.getMessage(), e);
		}
		return null;
	}
	...
	/**
	 * 根据appName和应用机器的信息，从ZK中查询所有的系统规则，返回的格式为map，key为单个系统规则的ID，value为单个系统规则的实体
	 * 
	 * @param app
	 * @return
	 */
	...
	private Map<Long, T> findBy(String app) {
		...
		String zkPath = ZookeeperConfigUtils.getSystemRuleZkPath(String.valueOf(app));
		try {
			...
			log.error("根据zkPath：" + zkPath + " 查询系统规则发生异常：" + e.getMessage(), e);
		}
		return null;
	}
