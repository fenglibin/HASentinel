...
import com.alibaba.csp.sentinel.dashboard.datasource.entity.rule.SystemRuleEntityV2;
...
public class SystemControllerV2 {
	...
	public Result<List<SystemRuleEntityV2>> apiQueryMachineRules(String app, String ip, Integer port) {
		if (StringUtil.isEmpty(app)) {
			return Result.ofFail(-1, "app can't be null or empty");
		}
		try {
			List<SystemRuleEntity> list = repository.findAllByApp(app);
			if (list == null || list.size() == 0) {
				return Result.ofSuccess(null);
			}
			List<SystemRuleEntityV2> result = new ArrayList<SystemRuleEntityV2>();
			list.forEach(r -> {
				result.add(r.getSystemRuleEntityV2());
			});
	...
	public Result<SystemRuleEntityV2> apiAdd(String app, String ip, Integer port, Double highestSystemLoad,
		...
		return Result.ofSuccess(entity.getSystemRuleEntityV2());
	}

	@GetMapping("/save.json")
	@AuthAction(PrivilegeType.WRITE_RULE)
	public Result<SystemRuleEntityV2> apiUpdateIfNotNull(Long id, String app, Double highestSystemLoad,
			Double highestCpuUsage, Long avgRt, Long maxThread, Double qps) {
		if (id == null) {
