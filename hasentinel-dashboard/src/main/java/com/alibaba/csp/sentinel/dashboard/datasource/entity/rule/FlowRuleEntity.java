...
public class FlowRuleEntity implements RuleEntity {
	...
	public FlowRuleEntityV2 getFlowRuleEntityV2() {
		FlowRuleEntityV2 rule = new FlowRuleEntityV2();
		rule.setApp(this.app);
		rule.setClusterConfig(this.clusterConfig);
		rule.setClusterMode(this.clusterMode);
		rule.setControlBehavior(this.controlBehavior);
		rule.setCount(this.count);
		rule.setGmtCreate(this.gmtCreate);
		rule.setGmtModified(this.gmtModified);
		rule.setGrade(this.grade);
		rule.setId(String.valueOf(this.id));
		rule.setIp(this.ip);
		rule.setLimitApp(this.limitApp);
		rule.setMaxQueueingTimeMs(this.maxQueueingTimeMs);
		rule.setPort(this.port);
		rule.setRefResource(this.refResource);
		rule.setResource(this.resource);
		rule.setStrategy(this.strategy);
		rule.setWarmUpPeriodSec(this.warmUpPeriodSec);
		return rule;
	}
