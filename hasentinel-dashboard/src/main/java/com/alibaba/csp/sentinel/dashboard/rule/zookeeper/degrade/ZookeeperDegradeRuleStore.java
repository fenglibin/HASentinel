...
public class ZookeeperDegradeRuleStore<T extends DegradeRuleEntity> implements RuleRepository<T, Long> {
	...
	public T save(T entity) {
		...
		if (processedEntity != null) {
			...
			try {
				...
				log.error("将app:" + entity.getApp() + " ID:" + entity.getId() + "日降级规则保存到ZK中发生异常：" + e.getMessage(), e);
			}
		}
	...
	public T delete(Long id) {
		...
		try {
			...
			log.error("根据ID：" + id + " 删除降级规则发生异常：" + e.getMessage(), e);
		}
		return null;
	}
	...
	public T findById(Long id) {
		...
		try {
			...
			log.error("根据ID：" + id + " 查询降级规则发生异常：" + e.getMessage(), e);
		}
		return null;
	}
	...
	/**
	 * 根据appName和应用机器的信息，从ZK中查询所有的降级规则，返回的格式为map，key为单个降级规则的ID，value为单个降级规则的实体
	 * 
	 * @param app
	 * @return
	 */
	...
	private Map<Long, T> findBy(String app) {
		...
		try {
			...
			log.error("根据zkPath：" + zkPath + " 查询降级规则发生异常：" + e.getMessage(), e);
		}
		return null;
	}
	...
	public void clearAll() {
		...
		try {
			...
			log.error("从ZK路径：" + degradeRuleRootPath + " 清除所有的降级规则发生异常：" + e.getMessage(), e);
		}
	}
